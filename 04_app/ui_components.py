...
# ✅ 누락된 guardian_required 를 포함한 최종 매핑 사전
TAG_MAP = {
    ...
    with st.sidebar:
        ...
        with st.expander("키즈카페 지도", expanded=True):
            if df is not None and not df.empty:
                ...
                if selected_gu != "서울시 전체": map_df = map_df[map_df["district"] == selected_gu]
                ...
                st.pydeck_chart(pdk.Deck(map_style='light', initial_view_state=pdk.ViewState(latitude=map_df["latitude"].mean() if not map_df.empty else 37.5665, longitude=map_df["longitude"].mean() if not map_df.empty else 126.9780, zoom=10.5), layers=[pdk.Layer('ScatterplotLayer', data=map_df, get_position='[longitude, latitude]', get_color='[242, 183, 5, 220]', get_radius=250, pickable=True)], tooltip={"html": "<b>{tooltip_name}</b><br>{tooltip_dong}", "style": {"backgroundColor": "#6B9DD4", "color": "white", "fontSize": "13px", "padding": "10px", "borderRadius": "8px", "maxWidth": "150px"}}), height=230) 
...
def get_message_html(role, content, source_docs=None):
    ...
    if role == "assistant" and source_docs:
        ...
        for doc in source_docs[:1]:
            name = doc.get("place_name", "키즈카페"); address = doc.get("address", "주소 정보 없음"); link = doc.get("booking_url", "https://yeyak.seoul.go.kr/")
            img_url = ""
            ...
            if isinstance(raw_feats, str): raw_feats = raw_feats.replace("[", "").replace("]", "").replace("'", "").replace('"', "").split(",")
            tags_html = "".join([f"<span style='color: #6B9DD4; padding: 2px 8px; border-radius: 4px; font-size: 0.75rem; font-weight: bold; background-color: #F4F8FD; margin-right: 5px; margin-bottom: 5px; display: inline-block; white-space: nowrap; border: 1px solid #6B9DD4;'>{TAG_MAP.get(f.strip().lower(), f'#{f.strip().replace(\"_\",\"\")}')}</span>" for f in raw_feats[:5] if f.strip()])

            # ✅ border-radius: 16px 복구 완료
            cards_html += f"""
            ...
            """
        if len(source_docs) > 1: cards_html += """<div style="margin-top: 12px; display: inline-block; background-color: #FFF4D6; border: 1px dashed #F2B705; color: #B28704; padding: 10px 16px; border-radius: 20px; font-size: 0.85rem; font-weight: 800; word-break: keep-all;">추천 질문: "2순위 후보도 보여줘" 라고 입력해보세요!</div>"""
        cards_html += "</div>"
    ...
    return f'<div class="bubble-container {cls}-msg">{display_content}</div>'.replace("\\n", "")