...
def get_message_html(role, content, source_docs=None, intent=None):
    ...
    if role == "assistant" and source_docs:
        ...
        # 2. 이동수단, 주차, 예약 등 정보 확인 시 -> 미니 카드 노출
        ...
        # 3. 그 외 단순 꼬리질문(place_detail 등) -> 카드를 렌더링하지 않음 (cards_html은 빈 문자열 유지)
        
    # 노리메이트 글씨 하얀색(white) 강제 적용
