...
def render_sidebar(df: pd.DataFrame):
    with st.sidebar:
        st.markdown(
            """<div style="padding: 0px 0 20px 0; text-align: center;">
            <span class="sidebar-logo">BEBENORI</span><br>
            <span style="color:#6B9DD4; font-size:0.85rem; font-weight: 800;">서울형 키즈카페 안심 가이드</span></div>""", unsafe_allow_html=True
        )
        ...
        with st.expander("키즈카페 지도", expanded=True):
            if df is not None and not df.empty:
                ...
                st.pydeck_chart(pdk.Deck(
                    ...
                    tooltip={
                        "html": "<b>{tooltip_name}</b><br>{tooltip_dong}",
                        "style": {
                            "backgroundColor": "#6B9DD4", "color": "white", "fontSize": "13px", "padding": "10px", "borderRadius": "8px", "maxWidth": "150px"
                        }
                    }
                ), height=230) 
        ...
        st.markdown(
            ...
            <a href="https://yeyak.seoul.go.kr/" target="_blank" style="color: #6B9DD4; font-weight: 800; font-size: 0.95rem; text-decoration: none;">
            공식 사이트 예약하러 가기
...
def get_message_html(role, content, source_docs=None):
    ...
    if role == "assistant" and source_docs:
        ...
        if len(source_docs) > 1:
            cards_html += """
            <div style="margin-top: 12px; display: inline-block; background-color: #FFF4D6; border: 1px dashed #F2B705; color: #B28704; padding: 10px 16px; border-radius: 20px; font-size: 0.85rem; font-weight: 800; word-break: keep-all;">
                추천 질문: "2순위 후보도 보여줘" 라고 입력해보세요!
            </div>
            """
        cards_html += "</div>"
    ...
    <div style="display: flex; flex-direction: column; align-items: center; min-width: 55px;">
        ...
        <div style="font-size: 0.6rem; font-weight: bold; color: {color if role=='user' else 'white'}; margin-top: 4px; text-align: center;">{label}</div>
    </div>
    """
    ...
    final_html = f'<div class="bubble-container {cls}-msg">{display_content}</div>'
